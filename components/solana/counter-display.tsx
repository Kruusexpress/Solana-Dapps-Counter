...
export function CounterDisplay() {
  ...
  const handleInitialize = useCallback(async () => {
    ...
    try {
      const instruction = await createInitializeInstruction(publicKey)
  ...
  const handleIncrement = useCallback(async () => {
    ...
    try {
      const instruction = await createIncrementInstruction(publicKey)
      const transaction = await buildTransaction(
        connection,
        instruction,
        publicKey
  ...
  const handleDecrement = useCallback(async () => {
    ...
    try {
      const instruction = await createDecrementInstruction(publicKey)
      const transaction = await buildTransaction(
        connection,
        instruction,
        publicKey
      ...
      <div className="flex flex-col items-center gap-8 py-16">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 max-w-sm text-sm text-muted-foreground leading-relaxed">
            Connect a Solana wallet to interact with the on-chain counter
            program on Devnet.
          </p>
        </div>
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-xl border border-border bg-card p-8">
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="flex h-32 w-32 items-center justify-center rounded-2xl border border-border bg-secondary">
            {loading ? (
